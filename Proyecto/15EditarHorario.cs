...
namespace Proyecto
{
    public partial class Form15 : Form
    {
        ...
        private void CargarRUTs()
        {
            ...
            string consultaSql = "SELECT rutProfesor FROM AsignacionesHorarios";
            ...
            using (SqlCommand comando = new SqlCommand(consultaSql, conexion))
            {
                try
                {
                    ...
                    while (lector.Read())
                    {
                        string rut = lector["rutProfesor"].ToString();
                        rutsUnicos.Add(rut);
                    }
                ...
                {
                    MessageBox.Show("Error al obtener los Rut: " + ex.Message);
                }
            }
        }
        ...
        private void cbx_rut_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (rutSeleccionado != null)
            {
                ...
                string consultaSqlIdHorario = "SELECT idAsignacion FROM AsignacionesHorarios WHERE rutProfesor = @rut";

                // Consulta SQL para obtener los datos de periodo y día desde la tabla Disponible según el profesor_rut
                string consultaSqlDisponible = "SELECT DISTINCT dias FROM AsignacionesHorarios WHERE rutProfesor = @rutProfesor";

                // Consulta SQL para obtener los datos de la tabla Curso
                string consultaSqlCursos = "SELECT DISTINCT idCurso FROM AsignacionesHorarios WHERE rutProfesores = @rutProfesor";
                ...
                using (SqlCommand comandoCursos = new SqlCommand(consultaSqlCursos, conexion)) // Nueva consulta para obtener los cursos
                {
                    try
                    {
                        ...
                        comandoDisponible.Parameters.AddWithValue("@rutProfesor", rutSeleccionado);
                        ...
                        {
                            int idHorario = Convert.ToInt32(lectorIdHorario["idAsignacion"]);
                        ...
                        {
                            string curso = lectorCursos["idCurso"].ToString();
        ...
        private bool PeriodoEstaAsignado(int idHorarioActual, string periodoSeleccionado, string diasSeleccionados)
        {
            try
            {
                ...
                using (SqlConnection conexion = new SqlConnection(cadenaConexion))
                {
                    ...
                    string consultaSql = "SELECT COUNT(*) FROM AsignacionesHorarios WHERE idAsignacion != @idAsignacion AND periodo = @periodo AND dias = @dias";

                    using (SqlCommand comando = new SqlCommand(consultaSql, conexion))
                    {
                        // Agregar los parámetros para el idHorario actual, el periodo seleccionado y el día seleccionado
                        comando.Parameters.AddWithValue("@idAsignacion", idHorarioActual);
        ...
        private void btnConfirmar_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Debe seleccionar un idAsignacion antes de confirmar los cambios.");
                return;
            }
        ...
        private void ActualizarHorario(int idHorario, string periodo, string curso, string dias)
        {
            try
            {
                ...
                using (SqlConnection conexion = new SqlConnection(cadenaConexion))
                {
                    ...
                    string consultaSql = "UPDATE AsignacionesHorarios SET periodo = @periodo, idCurso = @curso, dias = @dias WHERE idAsignacion = @idAsignacion";

                    using (SqlCommand comando = new SqlCommand(consultaSql, conexion))
                    {
                        // Agregar los parámetros
                        comando.Parameters.AddWithValue("@idAsignacion", idHorario);
        ...
        private void CargarDatosDataGridView()
        {
            try
            {
                ...
                using (SqlConnection conexion = new SqlConnection(cadenaConexion))
                {
                    ...
                    string consultaSql = "SELECT * FROM AsignacionesHorarios WHERE rutProfesores = @rut";
        ...
        private void cbx_idHorario_DropDownClosed(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(cbx_idHorario.SelectedItem?.ToString(), out idHorarioSeleccionado))
            {
                ...
                string consultaSqlDisponible = "SELECT DISTINCT dias, periodo FROM Disponibilidad WHERE profesor_rut = @rut AND id_Disponibilidad = @idHorario";
        ...
        private void cbx_Dia_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (diaSeleccionado != null)
            {
                ...
                string consultaSqlPeriodo = "SELECT DISTINCT periodo FROM Disponibilidad WHERE profesor_rut = @rut AND dias = @dia";
